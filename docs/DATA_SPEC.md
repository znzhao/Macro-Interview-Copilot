...
**`tier` and `verification_level` are independent axes.** `tier` is *governance* — who may edit it and who may see it. `verification_level` is *provenance* — how much you should trust that this is a real interview question. A community-tier question can legitimately be `verified_interview` if the submitter supplied a real source. Both are always displayed.
...
`(question_id, user_id)` composite PK, `value smallint` (+1 only in v1), `created_at`. A trigger maintains `questions.upvotes`.
...
| `question_votes` | all | own row only |
...
`interview_turns` has no `user_id` column, so its policy joins through `interview_sessions` — this is the easiest policy to get wrong and is explicitly covered by tests.
...
`QuestionFilters` is a frozen model: `tiers`, `modules`, `topics`, `difficulties`, `institutions`, `target_roles`, `verification_levels`, `min_upvotes`, `favorited_only`, `unattempted_only`.
...
- **Export.** Settings produces a single JSON file: profile, sessions, turns, evaluations, notes, favorites, mastery, private questions.
- **Delete.** Account deletion cascades all user-owned rows. Community questions the user published are **anonymized** (`author_id → NULL`) rather than deleted, so other users' interview history stays interpretable. This is stated before confirmation, not after.
